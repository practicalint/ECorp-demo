...
#FIX regex  readonly JDK_DOWNLOAD_URL4=$(${curl_cmd} ${JDK_DOWNLOAD_URL3} | egrep -o "http\:\/\/download.oracle\.com\/otn-pub\/java\/jdk\/[7-8]u[0-9]+\-(.*)+\/jdk-[7-8]u[0-9]+(.*)linux-x64.${EXT}" | tail -1)
  readonly JDK_DOWNLOAD_URL4=$(${curl_cmd} ${JDK_DOWNLOAD_URL3} | egrep -o "https\:\/\/download.oracle\.com\/otn\/java\/jdk\/[7-8]u[0-9]+\-(.*)\/jdk-[7-8]u[0-9]+(.*)linux-x64.${EXT}" | tail -1)
...
#FIX no java download  download_jdk 8 rpm
  cp /tmp/jdk-8u211-linux-x64.rpm .
